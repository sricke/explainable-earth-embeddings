...
def make_loss(loss_type: str, logit_scale_temp: float = 0.07, lambda_alignment: float = None, sigma: float = None) -> nn.Module:
    logit_scale_init = math.log(1.0 / logit_scale_temp)
    if loss_type == "clip":
        return CLIPLoss(logit_scale_init=logit_scale_init)
    if loss_type == "concept":
        ...
        return ConceptLoss(lambda_alignment=lambda_alignment, sigma=sigma, logit_scale_init=logit_scale_init)
    if loss_type == "mse":
        return nn.MSELoss()
...
class CLIPLoss(nn.Module):
    def __init__(self, logit_scale_init: float = math.log(1 / 0.07), symmetric: bool = True):
        super().__init__()
    ...
    def _forward_asymmetric(self, text_features, location_features, gps_embeddings_queue: torch.Tensor = None):
        ...
        cosine_sim_location_to_text = text_features @ location_features.t()
        logits_location_to_text = logit_scale * cosine_sim_location_to_text

        gps_embeddings_queue = F.normalize(gps_embeddings_queue, dim=1)
        all_location_features = torch.cat([location_features, gps_embeddings_queue])
        cosine_sim_all_text = text_features @ all_location_features.t()
        logits_text_to_all_locations = logit_scale * cosine_sim_all_text

        labels = torch.arange(batch_size, device=device)
        loss_text = F.cross_entropy(logits_text_to_all_locations, labels)
        loss_location = F.cross_entropy(logits_location_to_text, labels)
...
class ConceptLoss(nn.Module):
    ...
    def __init__(self, lambda_alignment: float = 0.0, sigma: float = 1.0, logit_scale_init: float = math.log(1 / 0.07), clip_symmetric: bool = True):
        super().__init__()
        self.lambda_alignment = lambda_alignment
        self.sigma = sigma
        self.clip_loss = CLIPLoss(logit_scale_init=logit_scale_init, symmetric=clip_symmetric)
