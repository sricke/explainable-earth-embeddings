...
from utils import set_seed, build_optimizer, build_scheduler, EarlyStopping
from checkpoint import load_checkpoint, build_checkpoint_path
...
def get_args():
    ...
    p.add_argument("--text_finetune_mode", default=text_encoder("finetune_mode", "lora"))
    p.add_argument("--text_projection", default=text_encoder("projection", "linear"))
    p.add_argument("--text_proj_hidden_layers", type=int, default=text_encoder("proj_hidden_layers", 0))
    p.add_argument("--text_proj_hidden_features", type=int, default=text_encoder("proj_hidden_features"))
    p.add_argument("--text_nonlinearity", default=text_encoder("proj_nonlinearity"))
    p.add_argument("--precomputed_text_embeddings", action="store_true", default=text_encoder("precomputed", False))
    ...
    p.add_argument("--lora_last_n_layers", type=int, default=text_encoder("lora_last_n_layers", 8))

    # Location encoder
    p.add_argument("--loc_finetune_mode", default=location_encoder("finetune_mode", "only_proj"))
    p.add_argument("--precomputed_location_embeddings", action="store_true", default=location_encoder("precomputed", True))
    ...
    p.add_argument("--train_subsample_size", type=int, default=training("train_subsample_size"))
    ...
    p.add_argument("--accumulation_steps", type=int, default=training("accumulation_steps", 1))
    p.add_argument("--val_every_n_steps", type=int, default=training("val_every_n_steps", 500))
    p.add_argument("--scheduler", default=training("scheduler"))
    p.add_argument("--warmup_steps", type=int, default=training("warmup_steps", 0))
    p.add_argument("--num_val_checks_without_improvement", type=int, default=training("num_val_checks_without_improvement", -1))
...
def main():
    ...
    train_dataloader, val_dataloader = build_dataloaders(
        dataset_path, args.batch_size, args.num_workers, args.seed,
    ...
    model = build_model(
        args.text_encoder, args.location_encoder,
        args.text_projection, shared_dim,
        args.text_finetune_mode, args.loc_finetune_mode,
        args.text_proj_hidden_layers, args.text_proj_hidden_features,
    ...
    early_stopper = EarlyStopping(patience_steps=args.num_val_checks_without_improvement) if args.num_val_checks_without_improvement > 0 else None
    ...
    if args.resume:
        start_step, best_val = load_checkpoint(args.resume, model, criterion, optimizer, scheduler, device)
        if early_stopper:
            early_stopper.load_best(best_val)

    run_train(
        train_dataloader, val_dataloader, model, criterion, optimizer, device,
        save_path=checkpoint_path, args=args, scheduler=scheduler,
        accumulation_steps=args.accumulation_steps, max_steps=args.max_steps,
        val_every_n_steps=args.val_every_n_steps, early_stopper=early_stopper,
