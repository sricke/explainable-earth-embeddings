...
class SPLICE(nn.Module):
    ...
    def __init__(
        ...
        self.dictionary = (
            self.dictionary / self.dictionary.norm(dim=1, keepdim=True)
        ).to(self.device)
        ...
        if self.solver == "skl":
            self.l1_penalty = (
                l1_penalty / (2 * self.image_mean.shape[0])
    ...
    def recompose_image(self, weights):
        ...
        recon_image = torch.nn.functional.normalize(
            recon_image + self.image_mean, dim=1
        )
        return recon_image
