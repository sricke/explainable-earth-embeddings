...
class BatchTopKTrainer(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        print("Loss function, input shape is: ", x.shape)
        ...
        if not logging:
            ...
            return namedtuple("LossLog", ["x", "x_hat", "f", "losses"])(
                ...
                x_hat,
                f,
                {"l2_loss": l2_loss.item(), "auxk_loss": auxk_loss.item(), "loss": loss},
            )
    ...
    def update(self, step, x):
        ...
        x = x.to(self.device)
        loss_dict = self.loss(x, step=step, logging=True)
        loss = loss_dict.losses["loss"]
        # print(loss)
        loss.backward()

        loss_dict.losses["loss"] = loss.item()
