...
parser.add_argument(
    "--model", type=str, default="resnet50", choices=["satclip", "resnet50"]
)
parser.add_argument(
    "--weights", type=str, default="imagenet", choices=["imagenet", "moco"]
)
...
for perplexity in [3, 5, 7, 8, 10, 12]:
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            ...
            ax.set_yticks([])
            ax.set_title(
                f"x∈[{xlo:.1f},{xhi:.1f}]  y∈[{ylo:.1f},{yhi:.1f}]", fontsize=8
            )

            fig_cell, ax_cell = plt.subplots(figsize=(20, 16))
            _draw_images(
                ax_cell, view_xlo, view_xhi, view_ylo, view_yhi, zoom=cell_zoom
            )
            ax_cell.set_xlabel("t-SNE 1")
            ax_cell.set_ylabel("t-SNE 2")
            ax_cell.set_title(
                f"cell (r={r}, c={c})  x∈[{xlo:.1f},{xhi:.1f}]  y∈[{ylo:.1f},{yhi:.1f}]"
            )
