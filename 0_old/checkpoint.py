...
def save_checkpoint(path, step, model, criterion, optimizer, scheduler, best_val_loss, args, loc_queue=None, text_queue=None):
    torch.save({
        'step': step,
    ...
    for state in optimizer.state.values():
        for k, v in list(state.items()):
            if torch.is_tensor(v):
                state[k] = v.to(device)
    # support old checkpoints that stored 'global_optimizer_step' or 'epoch'
    step = ckpt.get('step', ckpt.get('global_optimizer_step', ckpt.get('epoch', 0)))
