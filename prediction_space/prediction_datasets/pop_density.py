"""Population density / population outcomes at point locations."""
...
def load_lat_lon_value(
    *,
    ...
    """
    Load ``lat``, ``lon``, ``population`` from ``~/data/prediction_tasks/pop_density`` (or ``data_dir``).

    If ``log_transform`` is True, values are replaced with ``log1p(max(value, 0))`` (natural log).
    """
    path = Path(data_dir).expanduser() if data_dir is not None else DEFAULT_PREDICTION_TASKS_DIR / "pop_density"
    lat, lon, value = load_outcomes_sampled_csv(path, value_col="population")
    if log_transform:
