"""WWF / Resolve biomes: fixed lat/lon grid + polygon labels."""
...
def load_lat_lon_value(
    *,
    ...
    value_col: str = "BIOME_NUM",
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Load ``lat`` / ``lon`` from ``~/data/prediction_tasks/all_WORLD_100000_lat_lon.csv``
    (or ``lat_lon_csv``), then assign ``value_col`` from the biome shapefile under ``data_dir``.

    Default ``data_dir`` is ``~/data/prediction_tasks/biome``.
    """
    shp_dir = Path(data_dir).expanduser() if data_dir is not None else DEFAULT_PREDICTION_TASKS_DIR / "biome"
...
def load_embeddings_and_values(
    ...
    **kwargs,
    ...
    """``lat``, ``lon``, biome label, and embedding matrix with one row per location."""
